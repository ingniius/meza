...
import { isEqual, isNil } from "@azem/utils/lodash";
...
/**
 * @publicApi
 * @description Used to make child item part of the group context.
 * Needs to be used in a component that is a child of a component that has the `useGroupableParent` composition enabled.
 */
...
/**
 * @publicApi
 * @name useGroupableParent
 * @description Used to make a component a group parent component.
 * Provides the registration / toggle functions to its group children.
 * @param state
 * @param options
 * @param group
 */
export function useGroupableParent(
  ...
  // Internal copy of the selection. This allows the composition to work without the state option being passed
  const internalSelection = ref<(number | string)[]>([]);

  // Uses either the internal state, or the passed in state. Will call the onSelectionChange handler if it's passed
  ...
  // Provide the needed functions to all children groupable components
  // @NOTE nested item groups will override the item-group namespace, making nested item groups possible
  provide(group, { register, unregister, toggle, selection });

  // Whenever the value of the selection changes, we have to update all the children's internal states
  // If not, you can have an activated item that's not actually active
  watch(selection, updateChildren, { immediate: true });

  // It takes a tick before all children are rendered, this will make sure the start state of the children matches the start selection
  ...
  // These aren't exported with any particular use in mind
  ...
  // Remove a child within the context of this group
  ...
  function toggleMultiple(item: GroupableInstance) {
    ...
    // Don't remove it if it's the last item and the mandatory option is set
    ...
    if (options?.max?.value && options.max.value !== -1 && selection.value.length >= options.max.value) {
      // Even though we don't alter selection, we should flush the internal active state of the children to make sure we don't have any invalid internal active states
      updateChildren();
      return;
    }
  ...
  // Converts the item reference into the value that's used in the selection
  // This value is either the index of the item in the items array (by default), or the custom value that's passed in the groupable composition
  function getValueForItem(item: GroupableInstance) {
    return item.value || items.value.findIndex((child) => item === child);
  }

  // Loop over all children and make sure their internal active state matches the selection array of the parent

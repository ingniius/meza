...
import { obj as isObj, str } from "./assertion";
 ...
 * @publicApi
 * @name compress
 * @description Compress any input object or array down to a minimal size reproduction in a string.
 */
export function compress(obj: Dict | Dict[]) {
  ...
  const getAst = (part: unknown | unknown[]): Token | NestedToken => {
    if (part === null) return { type: Types.NULL, index: Tokens.NULL };
    if (part === undefined) return { type: Types.UNDEFINED, index: Tokens.UNDEFINED };
    if (Array.isArray(part)) return ["@", ...part.map((subPart) => getAst(subPart))];

    if (part instanceof Date) {
      const value = encode(part.toJSON());
      if (strings.has(value)) return { type: Types.STRING, index: strings.get(value)! };

      const index = strings.size;
      strings.set(value, index);

      return { type: Types.STRING, index };
    }
    ...
    if (part === "") return { type: Types.EMPTY, index: Tokens.EMPTY };
    if (str(part)) {
    ...
    if (typeof part === "number" && Number.isInteger(part)) {
      ...
      if (integers.has(value)) return { type: Types.INTEGER, index: integers.get(value)! };

      const index = integers.size;
      integers.set(value, index);
      return { type: Types.INTEGER, index };
    }

    if (typeof part === "number") {
      if (floats.has(part)) return { type: Types.FLOAT, index: floats.get(part)! };

      const index = floats.size;
      floats.set(part, index);
      return { type: Types.FLOAT, index };
    }

    if (typeof part === "boolean") return { type: Types.BOOLEAN, index: part ? Tokens.TRUE : Tokens.FALSE };
...
export function decompress(input: string): unknown {
  ...
  const getDecompressed = (): Dict | Dict[] => {

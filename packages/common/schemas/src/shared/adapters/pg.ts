import type { Column, Overview, Table } from "@azem/types/schema";
...
/**
 * @internal
 * @description Converts Postgres default value to JS.
 ...
 * @internal
 */
export class Pg implements Inspector {
  knex: Knex;
  schema: string;
  ...
  async overview(): Promise<Overview> {
    ...
    const [columnsResult, primaryKeysResult] = await Promise.all([
      // Only select columns from BASE TABLEs to exclude views (Postgres views cannot have primary keys so they cannot be used)
      this.knex.raw(
        `
        SELECT c.table_name
          , c.column_name
    ...
    // Before we fetch the available geometry types, we'll have to ensure PostGIS exists in the first place
    // If we don't, the transaction would error out due to the exception in SQL, which we can't catch in JS
    const hasPostGIS =

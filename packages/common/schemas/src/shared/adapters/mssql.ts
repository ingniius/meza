...
export function rawColumnToColumn(rawColumn: RawColumn): Column {
  function parseMaxLength(rawColumn: RawColumn) {
    ...
    if (Number.isNaN(max_length) || rawColumn.max_length === null || rawColumn.max_length === undefined) return null;
 ...
 * @private
 */
export class MSSQL implements Inspector {
  knex: Knex;
  _schema?: string;
  ...
  async columnInfo(table?: string, column?: string) {
    ...
    if (column) return rawColumnToColumn(dbResult as RawColumn);

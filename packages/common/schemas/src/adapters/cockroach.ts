...
import { stripQuotes } from "@azem/utils/charset";
...
/**
 * @private
 * @desription Converts CockroachDB default value to JS.
 ...
 * @private
 */
export class Cockroach implements Inspector {
  knex: Knex;
  schema: string;
  ...
  /**
   * @description Set the schema to be used in other methods.
   */
  ...
  async overview(): Promise<Overview> {
    ...
    for (const column of columns) {
      ...
      if (column.table_name in overview === false) {
        overview[column.table_name] = { columns: {}, primary: null };
      }
  ...
  /**
   * @description List all existing tables in the current schema/database.
   */
  async tables() {
  ...
  /**
   * @description Get the table info for a given table. If table parameter is undefined, it will return all tables in the current schema/database
   */
  ...
  async tableInfo(table?: string) {
    ...
    if (table) {
      ...
      return { name: rawTable.table_name, schema: rawTable.table_schema, comment: rawTable.table_comment } as Table;
    }

    const records = await query;
    return records.map((rawTable: RawTable): Table => {
      return { name: rawTable.table_name, schema: rawTable.table_schema, comment: rawTable.table_comment };
    });
  }

  /**
   * @description Check if a table exists in the current schema/database.
   */
  ...
  /**
   * @description Get all the available columns in the current schema/database. Can be filtered to a specific table.
   */
  async columns(table?: string) {
    ...
    const records = await query;
    return records.map(({ table_name, column_name }) => ({ table: table_name, column: column_name }));
  }

  /**
   * @description Get the column info for all columns, columns in a given table, or a specific column.
   */
  ...
  async columnInfo(table?: string, column?: string) {
    ...
    const query = this.knex
      ...
      .select<Column[]>({ table: "f_table_name", name: "f_geometry_column", data_type: "type" })
      .from("geometries")
  ...
  /**
   * @description Check if the given table contains the given column.
   */
  ...
  /**
   * @description Get the primary key column for the given table.
   */
  ...
  async foreignKeys(table?: string) {
    ...
    const rowsWithoutQuotes = result.rows.map((row) => {
      // @ts-ignore

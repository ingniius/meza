...
import { extractMaxLength, extractType, stripQuotes } from "@azem/utils/charset";
 ...
 * @internal
 */
export class SQLite3 implements Inspector {
  knex: Knex;
  ...
  async tableInfo(table?: string) {
    ...
    records = records.map((table) => ({ name: table.name, sql: table.sql }));

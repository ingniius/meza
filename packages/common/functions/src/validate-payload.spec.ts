...
describe("#validatePayload", () => {
  ...
  it("should return an array of 1 when there errors with an _and operator", () => {
    const mockFilter = { _and: [{ field: { _eq: "field" } }] } as Filter;
    const mockPayload = { field: "it" };
    expect(validatePayload(mockFilter, mockPayload)).toHaveLength(1);
  });

  it("should return an array of 1 when there errors with an _or operator", () => {
    const mockFilter = { _or: [{ field: { _eq: "field" } }] } as Filter;
    const mockPayload = { field: "it" };
    expect(validatePayload(mockFilter, mockPayload)).toHaveLength(1);
  });

  it("should return an array of 1 when there errors with an _or containing _and operators", () => {
    const mockFilter = {
      _or: [{ _and: [{ a: { _eq: 1 } }, { b: { _eq: 1 } }] }, { _and: [{ a: { _eq: 2 } }, { b: { _eq: 2 } }] }],
    } as Filter;
  ...
  it("should return an empty array when there is no error for filter field that does not exist in payload ", () => {
  ...
  it("should return an array of 1 when there is required error for filter field that does not exist in payload and requireAll option flag is true", () => {
    const mockFilter = { field: { _eq: "field" } } as Filter;
    // intentionally empty payload to simulate "field" was never included in payload
    const mockPayload = {};
  ...
  describe("validates operator: _contains", () => {
    const mockFilter = { _and: [{ value: { _contains: "MATCH-EXACT" } }] };
  ...
  describe("validates operator: _icontains", () => {
    const mockFilter = { _and: [{ value: { _icontains: "match-insensitive" } }] };
    const options = { requireAll: true };

...
describe("#applyOptionsData", () => {
  it("should return an empty object if the options are empty", () => {
    expect(applyOptionsData({}, {})).toEqual({});
  });

  it("should return the unchanged options if there are no mustaches", () => {
    expect(
      applyOptionsData(
  ...
  it("should return the options with any raw template replaced by the value in scope", () => {
    expect(
      applyOptionsData(
        { str: "{{ num }}", arr: ["{{ arr }}", { null: null }], obj: { str: "{{ obj }}", num: 42 } },
        { num: 42, arr: ["foo", "bar"], obj: { foo: "bar" } },
  ...
  it("should return the options with any non-raw template rendered with the respective stringified values from the scope", () => {
    expect(
      applyOptionsData(
  ...
  it("should return the options with raw templates with null scope values as literal null and undefined scope values as string undefined", () => {
  ...
  it("should return the options with non-raw templates which reference null or undefined scope values as literal null and undefined strings", () => {
    expect(
...
describe("#optionToObject", () => {
  it("should return the option parsed from json if the option is a string", () => {
    expect(optionToObject('{ "foo": 42 }')).toEqual({ foo: 42 });
  });

  it("should return the unchanged option if it is not a string", () => {
    expect(optionToObject(["foo", 42])).toEqual(["foo", 42]);
  });
});

describe("#optionToString", () => {
  it("should return the option stringified to json if it is an object or array", () => {
    expect(optionToString({ foo: 42 })).toBe('{"foo":42}');
  });

  it("should return the option converted to a string if it is not an object or array", () => {

...
import type { ConsoleInterface, WebSocketInterface } from "../index";
...
import { messageCallback } from "./utils/message-callback";
 ...
 * @publicApi
 * @name realtime
 * @description Creates a client to communicate with a Meza REST WebSocket.
 * @param config The optional configuration.
 */
export function realtime(config: WebSocketConfig = {}) {
  return <Schema>(client: MezaClient<Schema>) => {
    ...
    const debug = (level: keyof ConsoleInterface, ...data: any[]) =>
      config.debug && client.globals.logger[level]("[Meza SDK]", ...data);
    ...
    return {
      ...
      onWebSocket(event: WebSocketEvents, callback: (this: WebSocketInterface, ev: Event | CloseEvent | any) => any) {
        if (event === "message") {
          ...
          const updatedCallback = function (this: WebSocketInterface, event: MessageEvent<any>) {
            ...
            // try {
            //   @NOTE return callback.call(this,ON.parse(event.data));
            // } catch {
      ...
      sendMessage(message: string | Record<string, any>) {
        ...
        // @NOTE state.connection.sendON.stringify(message));
      },
        ...
        > {
          ...
          if (config.reconnect && reconnectState.active) {
            ...
            if (state.code === "open") {
              ...
              // @NOTE state.connection.sendON.stringify({ ...options, collection, type: 'subscribe' }));
